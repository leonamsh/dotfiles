...
(setq doom-font (font-spec :family "AnonymicePro Nerd Font Mono" :size 18)
        doom-variable-pitch-font (font-spec :family "AnonymicePro Nerd Font Mono" :size 18))
...
;; Set locale for Hunspell/Flyspell
(setenv "LANG" "pt_BR.UTF-8")
(setenv "LC_ALL" "pt_BR.UTF-8")
;; You might also want to set the Emacs-internal coding system, just in case
...
(after! lsp-mode
  (defun my/lsp-filter-diagnostics (diagnostics)
    "Remove semicolon warnings from diagnostics."
    (seq-filter
     (lambda (diag)
       (let ((msg (gethash "message" diag)))
         (not (or (string-match-p "missing ;" msg)
                  (string-match-p "Missing semicolon" msg)
                  (string-match-p "Expecting ';'" msg)))))
     diagnostics))

  (advice-add 'lsp-diagnostics--flycheck-callback :filter-return #'my/lsp-filter-diagnostics))
...
;; ?? Remapeia automaticamente os modos para tree-sitter
...
;; Ativa formatação com prettier ao salvar (format-all ou lsp)
(setq +format-on-save-enabled-modes
      '(js-ts-mode tsx-ts-mode typescript-ts-mode css-ts-mode json-ts-mode html-ts-mode web-mode))

;; ?? Usa o prettier como formatador se não quiser depender do lsp pra isso
(after! format-all
  (set-formatter! 'prettier "prettier --stdin-filepath" :modes '(js-ts-mode tsx-ts-mode typescript-ts-mode)))

;; ??? Ajustes para LSP e LSP-UI
(after! lsp-mode
  (setq lsp-headerline-breadcrumb-enable t
...
;; ?? Node.js path (as vezes necessario para LSP encontrar as libs globais)
(setq lsp-clients-typescript-server-args '("--stdio"))
(setenv "PATH" (concat (getenv "PATH") ":/usr/local/bin")) ;; ou onde teu node esta
